...
class HCP3DCondDataset(Dataset):
    def __init__(
        ...
        sample_smooth_img = np.load(
            self.cond_img_path / item.replace(".nii.gz", ".npy")
        )
        print(
        ...
        self.fractions = np.load(
            "/home/sz9jt/projects/generative_brain/src/python/datasets/fractions.npy"
        )
    ...
    def __getitem__(self, index):
        ...
        cond_image = np.load(
            self.cond_img_path / (self.files[index].replace(".nii.gz", ".npy"))
        )
...
class TomsHCP3DCondPatchDataset(Dataset):
    ...
    def __init__(
        ...
        for i in range(len(split_df)):
            gt_path = (
                self.root_dir / split_df["subject"].iloc[i] / "T1w" / self.gt_img_name
            )
            cond_path = (
                self.root_dir / split_df["subject"].iloc[i] / "T1w" / self.cond_img_name
            )
        ...
        if self.patch_size[0] == 64:
            ...
            assert (
                self.fractions.shape == sample_gt_img.shape
            ), "Fractions shape mismatch"
            ...
            print(
                f"Percentage of patches >= threshold: {self.coords.shape[0] / (self.fractions.size) * 100:.2f}%"
            )
