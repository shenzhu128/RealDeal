...
class UpsampleSubConv(nn.Module):
    """
    Convolution layer that is a substitute for upsampling layer.
    This won't change the spatial dimensions of the input, but will add a convolution layer.
...
class DownsampleSubConv(nn.Module):
    """
    Convolution layer that is a substitute for downsampling layer.
    This won't change the spatial dimensions of the input, but will add a convolution layer.
...
class Encoder(nn.Module):
    ...
    Args:
        ...
        use_flash_attention: if True, use flash attention for a memory efficient attention mechanism.
        downsample_control: whether to downsample at each layer in the encoder, length should be equal to num_channels - 1.
    """
    ...
    def __init__(
        ...
        downsample_control: Sequence[bool],
        ...
        self.downsample_control = downsample_control
        ...
        for i in range(len(num_channels)):
            ...
            if not is_final_block:
                if self.downsample_control[i]:
                    blocks.append(Downsample(spatial_dims=spatial_dims, in_channels=input_channel))
                else:
                    blocks.append(DownsampleSubConv(spatial_dims=spatial_dims, in_channels=input_channel))
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        reversed_downsample_control = list(reversed(downsample_control))
        ...
        for i in range(len(reversed_block_out_channels)):
            ...
            if not is_final_block:
                if reversed_downsample_control[i]:
                    blocks.append(
                        Upsample(
                    ...
                    blocks.append(UpsampleSubConv(spatial_dims=spatial_dims, in_channels=block_in_ch))
...
class AutoencoderKLDownsampleControl(nn.Module):
    """
    ...
    def __init__(
        ...
        use_convtranspose: bool = False,
        downsample_control: Sequence[bool] = (True, True, True),
    ) -> None:
        super().__init__()
        ...
        if len(num_channels) - 1 != len(downsample_control):
            raise ValueError("AutoencoderKL expects downsample_control being one less than num_channels")
