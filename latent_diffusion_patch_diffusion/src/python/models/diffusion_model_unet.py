...
class CoordinateMLP(nn.Module):
    """
    MLP for coordinate embedding. The coordinates for patches are of low dimension (2D or 3D).
    It is generally a good idea to train an additional embedding module to convert the coordinates
    into a higher-dimensional embedding.This is because raw coordinates (even if scaled) are
    low-dimensional and may not provide enough representational power for the model to effectively
    use them in the cross-attention mechanism.

    Args:
        input_dim: dimension of the low dimensional coordinates (2D x, y or 3D x, y, z)
        embed_dim: dimension of the output higher dimensional representation
        hidden_dim: dimension of the intermediate results
    """

    def __init__(self, input_dim, embed_dim, hidden_dim=64):
        super().__init__()
...
class ModifiedDiffusionModelUNetWithCoordinateEmbedding(nn.Module):
    """
    Unet network with timestep embedding and attention mechanisms for conditioning based on
    Rombach et al. "High-Resolution Image Synthesis with Latent Diffusion Models" https://arxiv.org/abs/2112.10752
    and Pinaya et al. "Brain Imaging Generation with Latent Diffusion Models" https://arxiv.org/abs/2209.07162
    ...
    Args:
        ...
        use_flash_attention: if True, use flash attention for a memory efficient attention mechanism.
        #
        coord_input_dim: dimension of the low dimensional coordinates (2D x, y or 3D x, y, z)
        coord_hidden_dim: dimension of the intermediate results
        # embedding_dim in the MLP should be the same dimension as the cross_attention_dim
    """
    ...
    def __init__(
        ...
        # The output should have the same dimension as cross_attention_dim
        # and the output will be used as context
    ...
    def forward(
        ...
        Args:
            x: input tensor (N, C, SpatialDims).
            timesteps: timestep tensor (N,).
            coordinates: context tensor (N, 1, CoordinateDim).
            class_labels: context tensor (N, ).
            down_block_additional_residuals: additional residual tensors for down blocks (N, C, FeatureMapsDims).
            mid_block_additional_residual: additional residual tensor for mid block (N, C, FeatureMapsDims).
        """
        ...
        # 3. compute coordinate embeddings
        """"Convert coordinates (N, 1, CoordinateDim) to higher dimensional embedding (N, 1, ContextDim)"""
        if coordinates is not None:
            context = self.coord_mlp(coordinates)
