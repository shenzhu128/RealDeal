...
if __name__ == "__main__":
    ...
    ds = datasets.hcp3d_uncropping_ds.TomsHCP3DTestPatchUncroppingGenerated(args.data_path)
    ...
    data = ds[0]
    print("Using data", args.data_path, flush=True)
    ...
    diffusion_model.to(device)
    # TODO Cannot use eval(), somehow the behavior of the group norm is different
    ...
    tmp = args.data_path.split("/")[-1].replace(".nii.gz", "")
    tmp = f"ckpt{str(args.modified_group_norm_ckpt)}_generated_{tmp}"
