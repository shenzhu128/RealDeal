...
def main(args):
    ...
    if run_dir.exists() and (run_dir / "lastest_ckpt.pth").exists():
        resume = True
    else:
        resume = False
    ...
    val_ds = get_obj_from_str(config.dataset.target)(**config.dataset.params, type="val")
    ...
    if resume:
        ...
        checkpoint = torch.load(str(run_dir / "lastest_ckpt.pth"))
