...
def log_ldm_sample_unconditioned(
    ...
    # ncol: number of latent code dimension
    png_outpath = run_dir / "sample-png"
    png_outpath.mkdir(exist_ok=True)
    npy_outpath = run_dir / "sample-npy"
    npy_outpath.mkdir(exist_ok=True)

    nrow = len(latent)
    ncol = len(latent[0])
    x_slice = latent.shape[2]
    x_slice = x_slice // 2 - 2
    ...
    for i in range(nrow * ncol):
        ...
        ax.axis("off")
        ax.imshow(latent[i // ncol, i % ncol, x_slice].cpu().numpy(), cmap="gray")
    plt.savefig(str(png_outpath / f"sample_epoch{epoch}.png"), bbox_inches="tight")
    plt.close()

    np.save(str(npy_outpath / f"sample_epoch{epoch}_fixscale{scale_factor}.npy"), latent.cpu().numpy())
